        ...
        void graphEventProc()
        {
            ...
            while (SUCCEEDED (mediaEvent->GetEvent (&ec, &p1, &p2, 0)))
            {
                ...
                switch (ec)
                {
                    ...
                    case EC_USERABORT:
                        {
                            // only close for total failure states
                            // partial failure states are typically recoverable
                            HRESULT result = (HRESULT)(p1 & 0xFFFFFFFF);
                            if (result != 0x8889000A && // AUDCLNT_E_DEVICE_IN_USE
                                result != 0xC00D002F) // NS_E_INVALID_DATA
                            {
                                doClose = true;
                            }
                ...
                if (doClose)
                {
                    // this will cause this class to be destructed, at which
                    // point mediaEvent should not be referenced.
                    // therefore we break out of the while loop so that we
                    // don't continue trying to access mediaEvent.
                    component.close();
                    break;
                }
            }
